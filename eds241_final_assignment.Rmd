...
In the code chunk below, linear regressions return the average difference in outcomes between houses near and far from the incinerator. 
...
The results indicate that on average, houses near the incinerator have an area `r round(mod1_1978$coefficient[2], digits = 5)` square feet smaller than houses far from the incinerator. 

On average, houses near the incinerator have `r round(mod2_1978$coefficient[2], digits = 7)` fewer square feet of lot compared to houses far from the incinerator. 

On average, houses near the incinerator have an inflation-adjusted sales price that is  `r round(mod3_1978$coefficient[2], digits = 7)` less than houses far from the incinerator. 

While these are the returned average differences in 1978 between houses near and far from the incinerator, none of the Beta 1 values, which represent the average difference between the two groups, are significant below the 0.1 significance level. 
...
Based on the observed differences in (b), the estimate in (a) is likely to be biased downward (overstate the negative effect of the incinerator on housing values), because there are variables that are correlated with whether or not a home is near or far from the incinerator. 
